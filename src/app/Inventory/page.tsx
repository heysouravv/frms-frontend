  ...
  return (
    ...
    <div className='p-8 flex justify-start flex-col gap-y-3 items-start w-full'>
      ...
      <div className='flex justify-start items-center gap-x-5'>
        ...
        <img src='/assets/Icons/rightArrowLightGray.svg' className='w-2' />
        <p className='font-primaryOrange px-2 py-2 font-medium rounded-md bg-[#FFF1EB]'>Inventory</p>
      </div>

      {/* Inventory Management */}
      <div className='flex justify-between  items-center w-full'>
<div>
    <p className='font-semibold lg:text-2xl text-[#101828]'>Inventory and Stock Management</p>
</div>
...
<div><p className='font-[#101828] font-semibold text-lg'>Quick Access</p></div>
    ...
    {productDetails.map((detail) => (
      <div className='flex justify-between bg-white border border-[#DADBDF] rounded-md px-5 py-5 w-full items-center flex-row'>
        <div className='flex justify-start gap-1 items-start flex-col'>
          <p className='text-base font-semibold font-primaryOrange'>{detail.title}</p>
          <p className='text-sm font-medium'>{detail.description}</p>
        </div>
        <div><img src='/assets/Icons/rightArrowOrange.svg' className='w-2' /></div>
      </div>
    ))}
  </div>
<Table/>
