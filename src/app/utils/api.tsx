...
export const loginUser = async (email: string, password: string) => {
  const apiEndpoint = "https://api.ifsmserp.com/login/";
  try {
...
export const makePostRequest = async (body: object) => {
  const apiEndpoint = `https://api.ifsmserp.com/products/`;
...
export const fetchData = async (
  ...
  try {
    const response = await fetch("https://api.ifsmserp.com/products/", {
      method: "GET",
      headers: {
...
export const fetchTrackChangeData = async (
  setProducts: (arg0: any) => void,
  setIsLoading: (arg0: boolean) => void,
) => {
  const token = Cookies.get("token");
  try {
    const response = await fetch(
      "https://api.ifsmserp.com/product-price-history/",
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`, // Use the token from Cookies
...
//  const token = Cookies.get("token");
//  try {
//    const response = await fetch(`https://api.ifsmserp.com/products/${productId}/`, {
//      method: "PUT",
//      headers: {
//        'Content-Type': 'application/json',
  ...
  const updateProduct = async (productId: any, productData: any) => {
    ...
    try {
      const res = await axios.put(
        `https://api.ifsmserp.com/products/${productId}/`,
        productData,
        {
          headers: {
            "Content-Type": "application/json",
